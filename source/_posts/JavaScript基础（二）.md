...
（4）JSON
JSON（JavaScript Object Notation）：JS对象表示方式，特殊格式的字符串，能被任意语言识别，可转换为任意语言中的对象，主要用于开发中数据的交互；
要点：
&#8195;&#8195;1）JSON字符串中的属性名必须加双引号
&#8195;&#8195;2）JSON分类：对象{}、数组[]；
&#8195;&#8195;3）JSON中允许的值：字符串、数值、布尔值、null、对象（不包括函数）、数组；
&#8195;&#8195;4）JS中的工具类——JSON，可将一个JSON字符串转换为JS对象，也可以将一个JS对象转换为JSON，主要常用函数介绍：
&#8195;&#8195;&#8195;&#8195;JSON.parse()：可将JSON字符串转换为JS对象，参数：一个需要转换为JS对象的JSON字符串；
&#8195;&#8195;&#8195;&#8195;JSON.stringify()：可将一个JS对象转换为JSON字符串；
&#8195;&#8195;5）JSON工具类不支持IE7及以下浏览器；
&#8195;&#8195;6）JS中的eval()函数可用于执行一段字符串形成的JS代码并将执行结果返回，要点：
&#8195;&#8195;&#8195;&#8195;若执行的字符串中含有"{}"，eval()函数会将{}内的内容当作代码块处理；若不想被当作代码块处理，可以在"{}"前后加上括号,例如："({...})"；